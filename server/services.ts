...
export const isStaff = (role: Role) => role === 'instructor' || role === 'admin';
...
/** Notify the instructor who owns this course, plus administrators. */
export function notifyCourseStaff(course: Course, n: Omit<Notification, 'id' | 'userId' | 'createdAt' | 'read'>) {
  const targets = db().users.filter((u) => u.active && (u.id === course.createdBy || u.role === 'admin'));
...
export function maybeIssueCredential(user: UserRecord, course: Course): boolean {
  ...
  const now = Date.now();
  const mastery = flattenLessons(course).map((f) => lessonMastery(f, progressOf(enr, f.lesson.id), now));
  const pred = computePrediction(course, enr, mastery, 100);
...
/** Instructors (and the administrator) hear about learners who start to fall behind, at most once a week each. */
function generateStaffAlerts(staff: UserRecord) {
  const d = db();
  const now = Date.now();
  const week = Math.floor(now / (7 * 86_400_000));
  const courses = d.courses.filter((c) => c.status === 'published' && (staff.role === 'admin' || c.createdBy === staff.id));
  let created = 0;
...
export function generateNudges(user: UserRecord) {
  ...
  for (const enr of d.enrollments.filter((e) => e.userId === user.id)) {
    ...
    const due = snap.mastery.find((m) => m.reviewDue);
    if (due)
      push({
    ...
    const soon = d.officeHours.find(
      (o) => (o.courseId === 'all' || o.courseId === course.id) && Date.parse(o.startsAt) > now && Date.parse(o.startsAt) - now < 86_400_000,
    );
    if (soon)
      push({
        key: `oh-${soon.id}`,
        kind: 'office_hours',
        title: `Office hours within 24 hours: ${soon.title}`,
        body: new Date(soon.startsAt).toLocaleString(),
        link: { view: 'dashboard', courseId: course.id },
      });
  }
}
...
export function validateCourse(input: any, existing: Course | undefined, actor: UserRecord): Course {
  ...
  const finalExam = feQuestions.length
    ? {
        questions: feQuestions
          ...
          .map((q: any) => {
            const options = (Array.isArray(q?.options) ? q.options : []).map((o: unknown) => str(o, 500).trim()).filter(Boolean).slice(0, 6);
            return { id: str(q?.id, 60) || uid('q'), question: str(q?.question, 1000).trim(), options, correctIndex: num(q?.correctIndex, 0, Math.max(0, options.length - 1), 0), rationale: str(q?.rationale, 2000) };
          })
          .filter((q: any) => q.question && q.options.length >= 2),
  ...
  const now = nowIso();
  // Pricing is an administrator decision; instructors never see or change it.
  const price = actor.role === 'admin' ? Math.max(0, Math.min(99, Number(input.price) || 0)) : (existing?.price ?? 0);
  return {
