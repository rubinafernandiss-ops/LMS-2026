...
export function masteryState(score: number, started: boolean, attempted: boolean): MasteryState {
...
export function lessonMastery(f: FlatLesson, p: LessonProgress, now: number): TopicMastery {
  ...
  if (lesson.check.length > 0) {
    if (checks.length) {
      ...
      const checkComponent = 0.6 * best + 0.4 * latest;
      score = lesson.activity ? 0.8 * checkComponent + 0.2 * (p.activity ? 100 : 0) : checkComponent;
    } else if (started) {
      score = lesson.activity && p.activity ? 20 : 10;
    }
  ...
  let calibration: TopicMastery['calibration'];
  ...
  return {
    ...
    topic: lesson.topic,
    score,
    state: masteryState(score, started, attempted),
    bestCheck: best,
    firstCheck: first,
...
export function computeEngagement(args: {
  ...
  const { userId, courseId, events, officeHours, enrollment, now } = args;
  ...
  const relevantSessions = officeHours.filter(
    (o) => (o.courseId === 'all' || !courseId || o.courseId === courseId) && Date.parse(o.startsAt) + o.durationMin * 60_000 < now,
  );
  const sessionsOffered = relevantSessions.length;
  const sessionsAttended = relevantSessions.filter((o) => o.attended.includes(userId)).length;
  const attendanceRate = sessionsOffered ? round((sessionsAttended / sessionsOffered) * 100) : null;
  ...
  if (attendanceRate !== null) parts.push([attendanceRate / 100, 10]);
...
export function hypotheticalEngagement(base: EngagementMetrics, minutesPerDay: number, daysPerWeek: number): number {
  ...
  if (base.attendanceRate !== null) parts.push([base.attendanceRate / 100, 10]);
  const wsum = parts.reduce((a, [, w]) => a + w, 0);
...
export function computePrediction(
  ...
  const comprehension = mean(firsts);
  ...
  const attemptedMastery = mean(mastery.filter((m) => m.bestCheck !== null).map((m) => m.score));
  const finalBase = attemptedMastery ?? prior;
  ...
  return {
    ...
    comprehension: comprehension === null ? null : round(comprehension),
...
export function computeNextStep(args: {
  ...
  if (weak) {
    return {
      ...
      reason: `Your mastery here is ${weak.score}%. A short review and a retake is the fastest way to raise your predicted grade.`,
      minutes: 8,
      label: 'Review & retake',
  ...
  if (hasFinal && !passedFinal) {
    ...
    return {
      ...
      minutes: 15,
      label: 'See office hours',
      action: { kind: 'office_hours', courseId: cid },
      where: 'Support',
    };
  }
...
export function computeRecommendations(args: {
  ...
  const { course, enrollment, mastery, engagement, prediction, officeHours, now } = args;
  ...
  const upcoming = officeHours
    .filter((o) => (o.courseId === 'all' || o.courseId === cid) && Date.parse(o.startsAt) > now)
    .sort((a, b) => Date.parse(a.startsAt) - Date.parse(b.startsAt))[0];
  if (upcoming && prediction.passConfidence < 70 && prediction.evidence !== 'none') {
    recs.push({
      id: `oh-${upcoming.id}`,
      priority: 'soon',
      title: `Join office hours: ${upcoming.title}`,
      detail: 'Your pass confidence is below 70%. A live session is one of the quickest ways to get unstuck.',
      minutes: upcoming.durationMin,
      action: { kind: 'office_hours', courseId: cid },
    });
  }
...
export function computeRisk(args: {
  ...
  if (weak.length >= 2) factors.push({ label: `${weak.length} topics below 60% mastery`, severity: 'medium' });
...
export function buildSnapshot(args: {
  ...
  const { course, enrollment, userId, goal, events, officeHours, threads, now } = args;
  const flat = flattenLessons(course);
  const mastery = flat.map((f) => lessonMastery(f, progressOf(enrollment, f.lesson.id), now));
  const engagement = computeEngagement({ userId, courseId: course.id, events, officeHours, enrollment, now });
  ...
  const recommendations = computeRecommendations({ course, enrollment, mastery, engagement, prediction, officeHours, now });
