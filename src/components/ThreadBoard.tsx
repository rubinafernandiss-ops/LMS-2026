...
const roleTag = (r: string) => (r === 'instructor' ? 'Instructor' : r === 'admin' ? 'Admin' : null);
...
function Composer({
  ...
  // Learners write to their own instructor only; staff choose class-wide or private.
  const [audience, setAudience] = useState<'everyone' | 'instructor'>(staff && !askAdmin ? 'everyone' : 'instructor');
  ...
  useEffect(() => {
    if (open) {
      ...
      setAudience(staff && !askAdmin ? 'everyone' : 'instructor');
    }
  ...
  const send = async () => {
    ...
    setBusy(true);
    try {
      const r = await api('/threads', { body: { courseId, lessonId: lessonId || undefined, title, body, audience } });
      toast('success', askAdmin ? 'Sent to the administrator.' : staff ? 'Posted to the class.' : 'Sent to your instructor.');
      setTitle('');
      setBody('');
      onCreated(r.thread);
    } catch (e) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={askAdmin ? 'Ask the Administrator' : staff ? 'Post to the Class' : 'Ask Your Instructor'}
      footer={
        <>
          ...
          <Button loading={busy} disabled={tooShort} onClick={send} icon={<Send className="h-4 w-4" />}>
            {askAdmin ? 'Send to Administrator' : staff ? 'Post to the Class' : 'Send to My Instructor'}
          </Button>
        </>
      }
    >
      <div className="space-y-4">
        ...
        {(!staff || askAdmin) && (
          <div className="flex items-start gap-3 rounded-2xl border border-npurple/25 bg-npurple-soft/50 p-3.5">
            ...
            <p className="text-[13px] leading-snug text-ink-soft">
              This goes straight to <span className="font-semibold text-ink">{askAdmin ? 'the administrator' : 'your instructor'}</span>. No one else can see it.
            </p>
          </div>
        )}
        ...
        <div>
          ...
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={staff ? 'Reminder: office hours on Thursday' : 'How do I tell if an AI answer is reliable?'} />
...
/**
 * mode decides whose conversations this board shows:
 *  'default'          learner questions for the courses you can see
 *  'ask-admin'        an instructor's own questions to the administrator
 *  'admin-questions'  questions instructors have sent the administrator
 */
export type BoardMode = 'default' | 'ask-admin' | 'admin-questions';

export default function ThreadBoard({ courses, threadId, basePath, query, mode = 'default' }: { courses: BoardCourse[]; threadId?: string; basePath: string; query: URLSearchParams; mode?: BoardMode }) {
  const { user } = useSession();
  const staff = user.role !== 'learner';
  const askAdmin = mode === 'ask-admin';
  ...
  const threads = useMemo(() => {
    let t = data?.threads ?? [];
    if (mode === 'ask-admin') t = t.filter((x) => x.authorId === user.id && x.authorRole === 'instructor');
    else if (mode === 'admin-questions') t = t.filter((x) => x.authorRole === 'instructor');
    else if (staff) t = t.filter((x) => x.authorRole === 'learner');
    ...
    return t;
  }, [data, courseFilter, filter, user.id, mode, staff]);
      ...
      <div className={cx(selected && 'hidden lg:block')}>
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <Button onClick={() => setComposer(true)} icon={<Plus className="h-4 w-4" />} disabled={!courses.length}>
            {askAdmin ? 'Ask the Administrator' : staff ? 'New Post' : 'Ask My Instructor'}
          </Button>
          {courses.length > 1 && (
