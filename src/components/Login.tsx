...
export default function Login({ onAuthed }: { onAuthed: (u: PublicUser) => void }) {
  ...
  const ROLES = [
    ...
    { id: 'admin' as const, label: 'Administrator', icon: ShieldCheck, color: '#FF2E93', hint: 'Run the institute: people, courses and reports.', demo: { email: 'admin@usaii.org', password: 'Admin@2026' } },
  ];
                ...
                <AnimatePresence mode="wait">
                  {success ? (
                    ...
                    <motion.div key="form" exit={{ opacity: 0, y: -10 }}>
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-1.5 rounded-2xl bg-mist p-1.5" role="tablist" aria-label="Choose how you are signing in">
                        {ROLES.map((r) => {
                          const on = role === r.id;
                          return (
                            <button
                      ...
                      {role === 'learner' && <p className="mt-4 text-center text-xs text-ink-faint">Don’t have an account? Ask your instructor or administrator.</p>}
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
