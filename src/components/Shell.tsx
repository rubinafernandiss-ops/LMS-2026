...
export function notificationPath(n: Notification, role: string): string {
  const l = n.link;
  if (!l) return role === 'learner' ? 'dashboard' : role === 'admin' ? 'overview' : 'cohort';
  if (role === 'learner') {
    switch (l.view) {
      case 'ask':
      ...
      case 'study':
        return l.courseId && l.lessonId ? `study/${l.courseId}/${l.lessonId}` : 'learning';
      case 'progress':
  ...
  switch (l.view) {
    ...
    default:
      return role === 'admin' ? 'overview' : 'cohort';
  }
}

/** Light or dark, available to learners, instructors and administrators alike. */
...
function ProfileMenu() {
  ...
  const roleLabel = user.role === 'admin' ? 'Administrator' : user.role === 'instructor' ? 'Instructor' : 'Learner';
  return (
    <div className="relative" ref={ref}>
  ...
  const sidebar = (animated: boolean) => (
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-4 scroll-thin" aria-label="Main">
        {nav.map((it) => renderItem(it, animated))}
      ...
      <div className="border-t border-line px-5 py-4 text-[11px] text-ink-faint">
        Signed in as <span className="font-semibold text-ink-soft">{user.role === 'admin' ? 'Administrator' : user.role === 'instructor' ? 'Instructor' : 'Learner'}</span>
      </div>
    </div>
  );
