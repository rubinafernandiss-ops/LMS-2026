...
import { Button, Card, Empty, PageHeader, Select, Textarea, cx, useToast } from '../components/ui';
...
function RatingCard({
  ...
  /** Shows the "would you recommend USAII courses?" question under the comment box. */
  ...
  const send = async () => {
    ...
    if (askRecommend && recommend === null) return toast('error', 'Please answer whether you would recommend USAII® courses.');
    setBusy(true);
    try {
      await api('/feedback/rating', {
        body: {
          ...
          stars,
          comment,
          ...(askRecommend && recommend !== null ? { recommend } : {}),
        },
      });
      setDone(true);
      setStars(0);
          ...
          <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-5 space-y-4">
            ...
            {askRecommend && (
              <div className="rounded-2xl border border-line bg-mist/50 p-4">
                <div className="text-sm font-semibold">Would you recommend USAII® Courses to others?</div>
    ...
    <Card className="relative overflow-hidden">
      ...
      {answer !== null && (
        <motion.p initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="mt-4 rounded-2xl bg-ngreen-soft px-4 py-2.5 text-sm font-semibold text-ngreen-ink">
          {answer ? 'Thank you. It means a lot that you would recommend us.' : 'Thank you for telling us. Your comments above help us put it right.'}
        </motion.p>
      )}
    </Card>
  );
