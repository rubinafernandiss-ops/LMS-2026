...
function FeedbackPrompt() {
  const { home, reload, readOnly } = useLearner();
  const toast = useToast();
  ...
  const send = async () => {
    ...
    setBusy(true);
    try {
      await api('/feedback', { body: { recommend, ease, comment: '' } });
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <Card className="relative">
          ...
          <div className="grid gap-5 md:grid-cols-[1fr_1fr_auto] md:items-end">
            <div>
              <div className="mb-2 font-semibold">Would you recommend this course?</div>
...
/** Shown only when the chance of passing drops below 70%: a live session is the fastest help. */
function OfficeHoursNudge() {
  const { home, active, readOnly, learner } = useLearner();
  const toast = useToast();
  const [busy, setBusy] = useState(false);
  const [registered, setRegistered] = useState(false);
  if (!active) return null;
  const p = active.snapshot.prediction;
  if (p.evidence === 'none' || p.passConfidence >= 70 || active.snapshot.credentialEarned) return null;
  const next = home.officeHours.find((o) => Date.parse(o.startsAt) + o.durationMin * 60_000 > Date.now() && (o.courseId === 'all' || o.courseId === active.course.id));
  const isIn = registered || (next ? next.registered.includes(learner.id) : false);
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap items-center gap-4 rounded-3xl border border-npurple/25 bg-gradient-to-r from-npurple-soft to-nblue-soft/70 p-5">
      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white text-npurple shadow-sm">
        <Video className="h-5 w-5" />
      </span>
      <div className="min-w-0 flex-1">
        <div className="font-bold">Get live help from your instructor</div>
        <div className="text-sm text-ink-soft">{next ? `${next.host} · ${next.title} · ${fmtDateTime(next.startsAt)}` : 'No session is scheduled yet. You can ask your instructor a question anytime.'}</div>
      </div>
      {!readOnly &&
        (next ? (
          <Button
            ...
            onClick={async () => {
              setBusy(true);
              try {
                await api(`/office-hours/${next.id}/register`, { body: {} });
                setRegistered(true);
                toast('success', 'You are registered. We will remind you.');
              } catch (e) {
                toast('error', (e as Error).message);
              } finally {
                setBusy(false);
          ...
          >
            {isIn ? 'Registered' : 'Register'}
          </Button>
        ) : (
          <Button variant="secondary" onClick={() => navigate('ask?new=1')}>
            Ask instructor
          </Button>
        ))}
    </motion.div>
  ...
  const sections = [
    ...
    <Collapse key="how" title="How to read this page">
      <ul className="space-y-1.5">
        ...
        <li>
          <b className="text-ink">Chance of passing</b> compares your expected grade with the pass mark. It gets more accurate as you take more quizzes.
        </li>
        <li>
