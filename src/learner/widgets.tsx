...
import type { NextAction, TopicMastery } from '../../shared/types';
...
/** Average mastery across topics that have been checked. */
export function understandingOf(item: CourseItem): number | null {
  const m = item.snapshot.mastery.filter((t) => t.bestCheck !== null);
    ...
    <Card>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl bg-mist px-4 py-3">
          <div className="text-xs text-ink-faint">Expected grade</div>
...
export function HabitsRow({ item }: { item: CourseItem }) {
  ...
  const tiles = [
    ...
    { label: 'Live sessions attended', value: e.sessionsOffered ? `${e.sessionsAttended} of ${e.sessionsOffered}` : 'None yet', color: '#00C77F' },
...
export function PlanCard({ item, readOnly }: { item: CourseItem; readOnly?: boolean }) {
  ...
  const sim = useMemo(() => simulateWhatIf(item.course, item.enrollment, s.engagement, s.mastery, minutes, days, now), [item, s, minutes, days, now]);
    ...
    <Card>
      ...
      <div className="mt-4 space-y-5">
        {item.course.modules.map((m) => {
          const topics = item.snapshot.mastery.filter((t) => t.moduleId === m.id);
          return (
            <div key={m.id}>
...
function TopicRow({ t, courseId, readOnly }: { t: TopicMastery; courseId: string; readOnly?: boolean }) {
